...
namespace openstars
{
	namespace storage
	{
		...
		public:
			...
			static openstars::storage::AbstractKVStorage *createMongo(std::map<std::string, std::string> &aOptions)
			{
				std::string aHost = aOptions["host"];
				int aPort = Poco::NumberParser::parse(aOptions["port"]);
				...
				if ((aPort <= 0 || aHost == "") && (zkServer == "" || zkBasePath == ""))
					return NULL;
				openstars::storage::MongoKVStorage *aDb = new openstars::storage::MongoKVStorage(aHost, aPort,"kvdb","kv");
